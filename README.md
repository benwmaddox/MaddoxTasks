...
`agent issues` includes a deterministic `repositories` array derived from `repo:` labels. `agent claim` and `agent next` order work hierarchically: task families are ordered by their top-level root priority and sequence, then each family is traversed child-first with siblings ordered by priority and sequence. For `agent next`, `Active` is preferred before `Next` when candidates have the same priority. A child is considered using only its own repositories, so a reserved parent does not block a disjoint child, and a reserved child does not block a later claimable sibling. `agent claim` atomically selects the first eligible `Next` issue with a repository not reserved by `Active` or `ReadyForReview` work, changes it to `Active`, and prints the selected issue JSON. `agent next` reports the first `Active` or `Next` issue in the same hierarchy order. Both commands handle missing or cyclic parent links deterministically. Claim prints `null` when no repository-backed issue is available; scheduled runners should stop for that hour. `--dry-run` performs the same claim selection without changing the database.
