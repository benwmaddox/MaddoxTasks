...
public sealed class WorkerHost
{
        ...
        foreach (var mutation in plan.Mutations)
        {
            switch (mutation.Type)
            {
                ...
                case "SetRepositoryLabels":
                    ...
                    break;
                case "SetBlockers":
                    await RunRequiredCommandAsync(new { type = "SetBlockers", issueId = mutation.IssueId, blockedBy = mutation.BlockedBy }, null, ct);
                    break;
                case "ChangeStatus":
    ...
    public static string BuildResearchPrompt(TaskDto sourceTask, string snapshotPath)
        => "You are the Maddox blocked-task recovery researcher. Investigate the selected source task identified below. Parse the worker-supplied snapshot locally to read its full description and comments and identify the current blocker. The snapshot contains all current task records, including parents, children, dependencies, and terminal work, so verify current state instead of trusting a stale blocker comment. Read selected fields or recent comments in bounded chunks if histories are long; do not load the entire file into the prompt.\n\n"
            ...
            + "Actively route dependency blockers, but do the minimum task-ledger work necessary. Before proposing any CreateIssue, cluster the evidence by normalized diagnostic, API contract, and verified root cause. Prefer one upstream owner task for a shared compiler, runtime, service, or tooling defect. Create a consumer-repository task only when the evidence proves that repository needs its own source or configuration migration; never create per-consumer workarounds for one proven upstream defect. If an explicitly required prerequisite task is missing, you may propose a narrowly scoped CreateIssue with the correct repository and parent. If a prerequisite exists, update or activate it instead of creating a duplicate. A coordination parent whose objective is fully satisfied by terminal children may be completed. A source whose implementation PR is merged may be completed only when the source acceptance criteria are actually satisfied; do not invent visual, device, credential, or other acceptance requirements absent from the task.\n\n"
            + $"Every research-created issue must start in Next, and one research result may create at most {ResearchPlanPolicy.MaximumCreatedIssues}. If the evidence appears to require a larger fan-out, create none of those issues: describe the proposed root-cause groups in findings, return stillBlocked, and request human review of the split. Each created issue description must state the exact observed failure and evidence, the smallest required change or action, allowed files/symbols/systems, explicit out-of-scope work, the cheapest first validation, and the stop condition. Direct the implementation worker to preserve existing behavior, avoid adjacent refactoring or modernization, treat generated vendor/dependency updates as mechanical, validate in escalating cost order, and route any independent newly revealed failure separately instead of fixing it in the same task. Validation requirements are evidence, not permission to expand implementation scope.\n\n"
            + "You may propose only these task-entry mutations: AddComment, UpdateDescription, ChangePriority, AddLabel, RemoveLabel, SetRepositoryLabels, SetBlockers on an existing task other than the source task, ChangeStatus on an existing task other than the source task, and CreateIssue. SetBlockers replaces the complete blocker set and must use existing task tokens. CreateIssue may include a blockedBy array. You may include repository labels on a newly created issue; the worker will create it and then apply those labels. Never directly change the source task's status or blocker set. Set outcome to completed only when the source objective is fully satisfied by current evidence or the proposed task-entry mutations and requires no repository implementation. Set outcome to unblocked when the normal worker can now attempt or repair the source task; zero mutations are allowed when the research itself establishes that. Otherwise set stillBlocked, with concrete evidence of the exact unavailable user input, credential, hardware, or authorization and the smallest action that would unblock it. Every mutation object must include every schema field; set fields that do not apply to that mutation type to null. Return JSON matching the supplied schema, with concise findings explaining the evidence and next step.\n\n"
            + "SOURCE TASK:\n"
            ...
            + "\n\nDEPENDENCY RULE: A task blocker is satisfied only when its current status is Done. Rejected, missing, and every other status remain unresolved. If a live snapshot shows an unresolved dependency, identify the blocker and recommend completing or removing it before implementation proceeds."
            + "\n\nRESEARCH MUTATION RULE: SetBlockers replaces the complete dependency set on an existing non-source task. CreateIssue may include blockedBy issue tokens. The ledger validates token resolution and cycles atomically.";
    ...
    private string BuildEnvelope(Job job, bool repair)
    {
        ...
        const string dependencyContext = "The task JSON blockedBy array is the current dependency snapshot. Only blockers with status Done are satisfied. If any blocker is unresolved, follow its blockerReasons guidance, do not start dependent implementation, and report the exact prerequisite that needs completion or removal.";
    ...
    {
        if (batch.Description is null && batch.Comments.Length == 0 && batch.BlockedBy is null) return string.Empty;
        return "HUMAN TASK UPDATES (ordered, authoritative delta):\n" + JsonSerializer.Serialize(new
        {
            descriptionReplacement = batch.Description,
            userComments = batch.Comments,
            blockerDependenciesReplacement = batch.BlockedBy,
            dependencyRule = "Only blockers with status Done are satisfied. Missing tasks and every other status remain unresolved."
        });
    }
    ...
    public static List<string> BuildInitialCodexArguments(Job job, string schema, string envelope, string? fallbackDirectory = null)
    {
        ...
        var arguments = new List<string> { "exec", "--approve-for-me", "--json", "--output-schema", schema, "-m", job.Model,
            "-c", $"model_reasoning_effort={job.Effort}",
            "--skip-git-repo-check", "-C", workingDirectory };
    ...
    public static List<string> BuildContinuationCodexArguments(Job job, string schema, string prompt)
    {
        ...
        return ["exec", "--approve-for-me", "resume", job.ThreadId, "--json", "--output-schema", schema, "-m", job.Model,
            "-c", $"model_reasoning_effort={job.Effort}",
    ...
    private const string ResearchResultSchema = """{"type":"object","properties":{"outcome":{"enum":["completed","unblocked","stillBlocked"]},"summary":{"type":"string"},"findings":{"type":"array","items":{"type":"string"}},"mutations":{"type":"array","items":{"type":"object","properties":{"type":{"enum":["AddComment","UpdateDescription","ChangePriority","AddLabel","RemoveLabel","SetRepositoryLabels","SetBlockers","ChangeStatus","CreateIssue"]},"issueId":{"type":["string","null"]},"comment":{"type":["string","null"]},"description":{"type":["string","null"]},"newPriority":{"type":["integer","null"],"minimum":1,"maximum":5},"label":{"type":["string","null"]},"newStatus":{"enum":["Backlog","Next","Active","Blocked","ReadyForReview","Done","Rejected",null]},"repositories":{"type":["array","null"],"items":{"type":"string"}},"title":{"type":["string","null"]},"priority":{"type":["integer","null"],"minimum":1,"maximum":5},"status":{"enum":["Next",null]},"parentId":{"type":["string","null"]},"blockedBy":{"type":["array","null"],"items":{"type":"string"}}},"required":["type","issueId","comment","description","newPriority","label","newStatus","repositories","title","priority","status","parentId","blockedBy"],"additionalProperties":false}}},"required":["outcome","summary","findings","mutations"],"additionalProperties":false}""";
}
