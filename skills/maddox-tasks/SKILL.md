...
2. Build structured command JSON (`CreateIssue`, `ChangeStatus`, `ChangePriority`, `AddLabel`, `RemoveLabel`, `SetRepositoryLabels`, `SplitIssue`, `UpdateDescription`, `AddComment`, `RequeueBlocked`). Repository reservations are labels in the canonical form `repo:<name>`; repository identity is case-insensitive. Use `SetRepositoryLabels` to atomically replace repository scope while preserving unrelated labels.
   ...
   - For blocked-task research, use `agent research-claim` (or `agent research-claim --dry-run`) to atomically select one eligible `Blocked` task. Its exact `maddox-research-worker` comment marker is the durable attempt record. Successful or still-blocked research uses the configured `researchCooldown` (14 days by default). A worker-authored comment beginning exactly `Research worker could not complete: ` after the latest exact attempt marker uses the short failure cooldown (one hour by default); spoofed actors/prefixes and legacy histories keep the long cooldown. Override these with `--cooldown` and `--failure-cooldown`, or agent JSON `cooldown` and `failureCooldown` TimeSpan strings. The worker gives the research Codex a read-only snapshot of the selected task plus current blocked-task context, permits read-only web research, and validates a closed set of Maddox task-entry mutations before applying them. Research must cluster shared root causes, prefer one upstream owner task, and create a consumer task only for a proven repository-owned migration. Created tasks always start in `Next`, are limited to three per result, and receive a minimal-repair scope envelope; larger proposed fan-outs remain findings for human review. The internal `CompleteResearch` command can move the source to `Next`, or to `Done` when ledger-only research fully completed the objective, only when its research marker exists and the source is still `Blocked`; it cannot overwrite a human status change.
